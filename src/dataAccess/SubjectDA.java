...
public class SubjectDA {
     ...
     */
    private static List getNames(int numberOfNames) {
        List<String> result = new ArrayList<String>();
        InputStream stream = ClassLoader.getSystemResourceAsStream("data/female-names");
        ...
        while (true) {
            ...
            if (line != null && result.size() < numberOfNames ) {
                result.add(line);
            }else {
                break;
            }
    ...
    public static List<Subject> generateRandomSubjects(int numberOfSubjects,
        ...
        List<String> names = getNames(numberOfSubjects);
        ...
        for (int index = 0; index < numberOfSubjects; index++) {
            subject = new Subject(names.get(index));
            value1 = Math.abs(numberOfPreferencesMean
                    + (int) (RNG.nextGaussian() * numberOfPreferencesDev));
            for (int index2 = 0; index2 < value1; index2++) {
                recipeName = recipes.get(Math.abs(RNG.nextInt() % numberOfRecipes)).getName();
                subject.addPreference(new Preference(recipeName, Math.abs(RNG.nextInt() % 5) + 1));
            }
            for (int index2 = 0; index2 < numberOfRefusedIngredients; index2++) {
                randomRecipe = recipes.get(Math.abs(RNG.nextInt() % numberOfRecipes));
                subject.addRefusedIngredient(randomRecipe.getIngredients().get(0).getName());
            }
